"""Serve the Day 19 experimental LoRA through a loopback-only HTTP gateway."""
...
from experimental_command_guard import validate_model_output
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self._send_json(
            ...
            {
                "ok": True,
                "service": "mindcraft-day19-experimental-gateway",
                "guard_profile": "day19_experimental",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the Day 19 experimental Mindcraft vLLM gateway.")
...
def main():
    ...
    print(f"Day 19 experimental gateway ready at http://{args.host}:{args.port}")
    print(f"Upstream vLLM: {args.vllm_url} | model: {args.model}")
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print("Shutting down experimental gateway.")
    finally:
        server.server_close()
