"""Evaluate the base model or a visual LoRA adapter on the held-out sheep/pig manifest."""
...
from vision_classifier_common import extract_label, prepare_image, read_manifest, user_messages
...
def predict(model, processor, image, device: str) -> tuple[str | None, str]:
...
def main():
    ...
    parser.add_argument("--report", type=Path, default=Path("reports/entity_eval.json"))
    ...
    for record in records:
        image = prepare_image(record["image_path"], args.max_image_side)
        prediction, generated_text = predict(model, processor, image, args.device)
        results.append(
            {
                ...
                "exact_match": prediction == record["label"],
                "sent_size": {"width": image.width, "height": image.height},
                # Kept only in the ignored local report to diagnose formatting failures.
    ...
    report = {
        ...
        "exact_match_accuracy": matched / len(results),
        "parse_ok_count": parse_ok,
        "results": results,
    }
    args.report.parent.mkdir(parents=True, exist_ok=True)
    args.report.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Exact entity accuracy: {matched}/{len(results)} = {report['exact_match_accuracy']:.1%}")
