"""Serve Day 21 pre-policy plus the existing experimental exact command guard."""
...
from experimental_command_guard import GuardResult, validate_model_output
from response_policy import decide_request
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self._send_json(
            ...
            {
                "ok": True,
                "service": "mindcraft-day21-policy-gateway",
                "guard_profile": "day19_experimental_plus_day21_policy",
    ...
    def do_POST(self):
        ...
        decision = decide_request(user_text)
        if not decision.model_required:
            guard = GuardResult(False, "blocked", decision.reply, "", f"day21_{decision.reason}")
            self._send_json(
                HTTPStatus.OK,
                {"user": user_text, "raw_model_output": "", "policy": decision.to_dict(), "guard": guard.to_dict()},
            )
            return

        try:
            raw_output = self.client.generate_raw(user_text)
        ...
        self._send_json(
            HTTPStatus.OK,
            {"user": user_text, "raw_model_output": raw_output, "policy": decision.to_dict(), "guard": guard.to_dict()},
        )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the isolated Day 21 Mindcraft policy gateway.")
    parser.add_argument("--vllm-url", default="http://127.0.0.1:8000/v1")
    parser.add_argument("--model", default="mindcraft-lora-v2")
...
def main():
    ...
    print(f"Day 21 policy gateway ready at http://{args.host}:{args.port}")
    print(f"Upstream vLLM: {args.vllm_url} | model: {args.model}")
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print("Shutting down Day 21 policy gateway.")
    finally:
        server.server_close()
