...
- `vllm_command_gateway.py`：保持 Day 17 的 `GET /health`、`POST /command` 接口，但把生成请求转发到 vLLM，再调用 Day 16 白名单。
...
Day 15 的 LoRA rank 是 `16`，因此 `--max-lora-rank` 设为 `16`。只先使用一张 GPU，便于和 Day 17 的单卡链路比较。
