...
仓库覆盖 Day 1 至 Day 22 的学习记录，但本 README 只聚焦目前仍可复核、可继续迭代的 Mindcraft 主线。早期目录保留为个人学习过程和脚本归档；原始学习语料、模型权重、缓存和运行记录均不在仓库中，也不作为可复现产物承诺。
...
Day 1-22 均已形成可复盘的学习记录。Day 20 已建立独立的网关挑战集，用于在不修改模型和白名单的前提下，评测旧能力回归、新能力泛化、模糊意图和应被拒绝的请求。首轮 28 条主指标测试取得 96.4% 通过率和 100.0% 的命令映射准确率；8 条不支持请求均未触发不安全执行，其中 7 条被严格拒绝，1 条回退为无害查询。当前有两条明确隔离的命令链路：Day 18 是已验证的正式命令服务；Day 19 是使用 v2 LoRA、8766 实验网关和临时白名单的能力扩展环境。两条链路都保持“模型生成、精确白名单校验、Mindcraft 执行、游戏反馈记录”的顺序，原始交互日志不会上传仓库。
...
为控制上下文占用，客户端仅在内存中等比缩小待发送图片，原图保持不变且不会进入仓库。模型被要求输出固定字段的紧凑 JSON；`vision_observation_guard.py` 会拒绝非 JSON、额外字段、多行内容和包含命令特征的结果。首轮真实单图请求已返回符合契约的观察对象并被守卫接受，但“格式已验证”不等于“视觉结论必然正确”：方块、实体和危险标签仍应视为待验证线索。详见 [Day 22 README](<学习计划/Day 22 多模态观察与安全辅助决策/README.md>)。
